import RakutenMA, { default_featset_zh, create_hash_func } from 'rakutenma';
import { getJson } from '../api/getJson';

export const rakutensample = () => {
  return (async () => {
    const sample_txt = "你下午去银行办汇款，等一会儿回来公司出行分公司，行不行？等一下！"

    let model = await getJson("./data/model_zh.json");

    let rma = new RakutenMA(model, 1024, 0.007812);  // Specify hyperparameter for SCW (for demonstration purpose)
    rma.featset = default_featset_zh;
    rma.hash_func = create_hash_func(15);
    return rma.tokenize(sample_txt);
  })()
}
